...
contract("Aouncoin", function(accounts) {
    ...
    it('approves tokens for delegated transfer', function() {
        return Aouncoin.deployed().then(function(instance) {
            tokenInstance = instance;
            // again, call is not writing to blockchain, just calling for testing
            return tokenInstance.approve.call(accounts[1], 100);
        }).then(function(success) {
            assert.equal(success, true, 'it returns true');
            // make actual event to check logs when call the function
    ...
    it('handles delegated token transfers', function(){
        return Aouncoin.deployed().then(function(instance) {
            ...
            // spendingAccount is calling this function and trying to spend more token than available
            return tokenInstance.transferFrom(fromAccount, toAccount, 9999, {from: spendingAccount});
        }).then(assert.fail).catch(function(error) {
            assert(error.message.indexOf('revert') >= 0, "cannot transfer larger than the balance");
            ...
            return tokenInstance.allowance(fromAccount, spendingAccount); // give it keys in mapping
        }).then(function(allowance) {
